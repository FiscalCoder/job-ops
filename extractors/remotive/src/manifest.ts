...
import { type RemotiveProgressEvent, runRemotive } from "./run";

function toProgress(event: RemotiveProgressEvent): ExtractorProgressEvent {
  ...
  return {
    ...
    detail: complete
      ? `Remotive: completed ${event.termIndex}/${event.termTotal} (${event.searchTerm}) with ${event.jobsFoundTerm ?? 0} jobs`
      : `Remotive: term ${event.termIndex}/${event.termTotal} (${event.searchTerm})`,
  };
}
